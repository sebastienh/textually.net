...
class StyloPrivacyPolicy extends React.Component {
    ...
    render() {
        ...
        const nodioPrivacyPolicy =  data.nodioPrivacyPolicy.edges[0].node

        return (
            <PageLocation path={["/", "nodio", "privacy-policy"]}>

                <SEO
                    title="Release Notes"
                    keywords={[`nodio`, `releases`, `notes`]}
                />
                <Flex mt={140} flexDirection={"row"} justifyContent={"center"}>
                    <Heading  color={"#D74E09"} fontSize={[5,6,8]}>Nodio Privacy Policy</Heading>
                </Flex>
                <DocumentationPageTemplate mt={[20, 10, 0]}>
                    <MarkdownContent post={nodioPrivacyPolicy} />
...
query {
  nodioPrivacyPolicy: allMarkdownRemark(
    sort: { fields: [frontmatter___date], order: DESC }, 
    filter: { fields: { 
      slug: { eq: "/nodio-privacy-policy/" } } }) {
    edges {
      node {
        htmlAst
      }
